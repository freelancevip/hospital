    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.doctors.update", [$doctor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('img') ? 'is-invalid' : '' }}" type="text" name="img" id="img" value="{{ old('img', $doctor->img) }}">
                @if($errors->has('img'))
                    <div class="invalid-feedback">
                        {{ $errors->first('img') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('speciality') ? 'is-invalid' : '' }}" name="speciality_id" id="speciality_id" required>
                    @foreach($specialities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('speciality_id') ? old('speciality_id') : $doctor->speciality->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
